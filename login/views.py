...
def forgot_password(request):
    if request.method == 'POST':
        ...
        if user_valid:
            ...
            message = render_to_string('login/reset-password-email.html', {
                'user': user,
                'domain': current_site.domain,
                'uid':urlsafe_base64_encode(force_bytes(user.pk)),
                'token':account_activation_token.make_token(user),
            })
            send_mail(subject, message, EMAIL_HOST_USER, [request.POST.get('email'),], fail_silently=True)    
...
def notifications(request):
    ...
    content = {'notifications': notifications_details, 'notification_list':notifications, 'from_users': from_users}
...
def settings(request):
    ...
    if valid_username:
        ...
        content = {'notifications': notifications, 'incorrect': 'An Account Already Has That Email',}
        return render(request, 'login/settings.html', content)
...
def send_verification_email(request, email):
    ...
    message = render_to_string('login/acc-email-verification.html', {
        'user': request.user,
        'domain': current_site.domain,
